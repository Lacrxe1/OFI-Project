...
Original file is located at
    https://colab.research.google.com/drive/1-QYLO_xhYq3oWl_ZwIeavmfxDzpfXoHO
"""
...
# Calculate delta_bid
bid_conditions = [
    df['bid_px_00'] > df['prev_bid_px'],  # Condition 1: Price went up
    df['bid_px_00'] == df['prev_bid_px'], # Condition 2: Price is the same
    df['bid_px_00'] < df['prev_bid_px']   # Condition 3: Price went down
]
bid_outcomes = [
    df['bid_sz_00'],                     # Outcome 1: Imbalance is the new size
    df['bid_sz_00'] - df['prev_bid_sz'], # Outcome 2: Imbalance is the change in size
    0                                    # Outcome 3: Orders were pulled, no contribution to OFI
]
...
ask_outcomes = [
    df['ask_sz_00'],                     # Outcome 1: Imbalance is the new size
    df['ask_sz_00'] - df['prev_ask_sz'], # Outcome 2: Imbalance is the change in size
    0                                    # Outcome 3: Orders were pulled, no contribution to OFI
]
...
# We will plot the first 30 data points
...
# Add a horizontal line at y=0 to represent the balance point
